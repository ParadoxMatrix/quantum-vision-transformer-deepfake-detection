...
class Parallel_VectorLoader:
    ...
    def get_RBS_parameters(self, x):
        ...
        for j in range(0, half):
            r2.append(torch.sqrt(torch.pow(x[:, :, 2 * j + 1] + 1e-4, 2) + torch.pow(x[:, :, 2 * j] + 1e-4, 2) + 1e-8)[:, :, None])
        r2 = torch.cat(r2, dim=2)
        parameters1 = []
        parameters2 = []
        for j in range(0, half):
            parameters1.append(torch.acos(torch.clamp(r1[:, :, j] / (r2[:, :, j] + 1e-8), min=-1., max=1.))[:, :, None])
